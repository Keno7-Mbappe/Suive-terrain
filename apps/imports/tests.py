...
class ExportKoboChoicesTests(TestCase):
    def setUp(self):
        ...
        Beneficiaire.objects.create(
            ...
            region="djibouti", institution=self.institution,
        )

    def test_ecrit_les_trois_csv_avec_le_bon_format(self, tmp_path=None):
        import tempfile
        with tempfile.TemporaryDirectory() as tmp:
            with override_settings(BASE_DIR=Path(tmp)):
                ...
                for nom in ("beneficiaires.csv", "institutions.csv", "cycles.csv"):
...
class GenerateKoboXlsformsTests(TestCase):
    def test_genere_les_trois_formulaires_valides(self):
        import tempfile
        ...
        with tempfile.TemporaryDirectory() as tmp:
            with override_settings(BASE_DIR=Path(tmp)):
                ...
                fichiers = ["suivi.xlsx", "satisfaction_beneficiaire.xlsx", "satisfaction_institution.xlsx"]
                for nom in fichiers:
                    chemin = dossier / nom
                    ...
                    self.assertIn("id_beneficiaire" if "institution" not in nom else "id_institution", noms_champs)
    ...
    def test_pyxform_accepte_les_formulaires_generes(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            with override_settings(BASE_DIR=Path(tmp)):
                call_command("generate_kobo_xlsforms", stdout=StringIO())
                dossier = Path(tmp) / "kobo_forms"
                for nom in ["suivi.xlsx", "satisfaction_beneficiaire.xlsx", "satisfaction_institution.xlsx"]:
...
class TraiterSoumissionTests(TestCase):
    ...
    def test_reessai_reussi_efface_le_message_d_erreur_precedent(self):
        soumission = KoboSoumission.objects.create(
            ...
            donnees_brutes={
                "id_beneficiaire": self.beneficiaire.pk, "vague": "m3",
                "date_du_contact": "2026-01-01", "issue_du_contact": "injoignable",
            },
        )
    ...
    def test_echec_enregistre_le_message_d_erreur(self):
        soumission = KoboSoumission.objects.create(
            ...
            donnees_brutes={"id_beneficiaire": "B-INCONNU", "vague": "m3", "issue_du_contact": "injoignable"},
        )
        reussite = traiter_soumission(soumission)
        soumission.refresh_from_db()
